...
public class QueryInsightsSettings {
    ...
    // TODO: Window Size should be dividable by 1 hour, or (1 hour % window size) == 0
    public static final int DEFAULT_WINDOW_SIZE = 60;
    /** Default min export interval for Query Insights exporters */
//    public static final TimeValue MIN_EXPORT_INTERVAL = new TimeValue(DEFAULT_WINDOW_SIZE, TimeUnit.SECONDS);
    public static final TimeValue MIN_EXPORT_INTERVAL = new TimeValue(1, TimeUnit.MINUTES);
     ...
     */
    public static final String TOP_N_LATENCY_QUERIES_LOCAL_INDEX_EXPORTER_PREFIX = TOP_N_LATENCY_QUERIES_PREFIX + ".exporter.local_index";

    /**
     * Boolean setting for enabling top queries by latency exporter
     */
    public static final Setting<Boolean> TOP_N_LATENCY_QUERIES_LOCAL_INDEX_EXPORTER_ENABLED = Setting.boolSetting(
        TOP_N_LATENCY_QUERIES_LOCAL_INDEX_EXPORTER_PREFIX + ".enabled",
        false,
        Setting.Property.Dynamic,
        Setting.Property.NodeScope
    );
     ...
     */
    public static final Setting<TimeValue> TOP_N_LATENCY_QUERIES_LOCAL_INDEX_EXPORTER_INTERVAL = Setting.positiveTimeSetting(
        TOP_N_LATENCY_QUERIES_LOCAL_INDEX_EXPORTER_PREFIX + ".interval",
        MIN_EXPORT_INTERVAL,
        Setting.Property.NodeScope,
        Setting.Property.Dynamic
    );
     ...
     */
    public static final Setting<String> TOP_N_LATENCY_QUERIES_LOCAL_INDEX_EXPORTER_IDENTIFIER = Setting.simpleString(
        TOP_N_LATENCY_QUERIES_LOCAL_INDEX_EXPORTER_PREFIX + ".index_name",
        "top_queries_since_" + System.currentTimeMillis(),
        value -> {
            if (value == null || value.length() == 0) {
                throw new IllegalArgumentException(
                    String.format(Locale.ROOT, "Invalid index name for [%s]", TOP_N_LATENCY_QUERIES_LOCAL_INDEX_EXPORTER_PREFIX + ".identifier")
                );
            }
        },
        Setting.Property.NodeScope,
